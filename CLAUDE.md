...
SATVOS is a multi-tenant file upload service written in Go. It provides tenant-isolated file management with JWT authentication, role-based access control (admin/member), and AWS S3 storage. The architecture follows the hexagonal (ports & adapters) pattern.
...
internal/
  ...
  domain/
    models.go                Tenant, User, FileMeta structs
  ...
  handler/
    ...
    file_handler.go          POST /files/upload, GET /files, GET /files/:id, DELETE /files/:id
...
db/migrations/               3 SQL migrations: tenants -> users -> file_metadata
```
